...
RSpec.describe Item, type: :model do
  ...
  context '出品できない場合' do
    ...
    it 'shipping_priceが空だと出品できない' do
      @item.shipping_price = ' '
      @item.valid?
      expect(@item.errors.full_messages).to include("Shipping price before type cast can't be blank")
    end
    it 'shipping_priceが300円以下だと出品できない' do
      @item.shipping_price = 200
      @item.valid?
      expect(@item.errors.full_messages).to include('Shipping price before type cast  is out of setting range')
    end
    it 'shipping_priceが9999999円以上だと出品できない' do
      @item.shipping_price = 100_000_000
      @item.valid?
      expect(@item.errors.full_messages).to include('Shipping price before type cast  is out of setting range')
    end
    it 'shipping_priceが半角数字以外が含まれると出品できない' do
      @item.shipping_price = '444４４'
      @item.valid?
      expect(@item.errors.full_messages).to include('Shipping price before type cast  is out of setting range',
                                                    'Shipping price before type cast is invalid. Input half-width characters')
    end
